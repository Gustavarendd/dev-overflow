...
export async function getSavedQuestion(params: GetSavedQuestionsParams) {
  try {
    ...
    const userSavedQuestions = await User.findOne({ clerkId }).populate({
      path: 'savedQuestions',
      match: query,
      options: {
        skip: skipAmount,
    ...
    const user = await User.findOne({ clerkId });
    const savedQuestions = userSavedQuestions.savedQuestions;

    const isNext =
      user.savedQuestions.length > skipAmount + savedQuestions.length;
